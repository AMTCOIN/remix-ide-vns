    ...
    var environmentEl = yo`
      <div class="${css.crow}">
        ...
        <div class=${css.environment}>
          ...
          <a href="http://www.vnscoin.org/" target="_blank"><i class="${css.infoDeployAction} fas fa-info"></i></a>
        </div>
      </div>
    `
    const networkEl = yo`
    ...
    selectExEnv.addEventListener('change', (event) => {
      ...
      this.settings.changeExecutionContext(context, () => {
        const modal = modalDialogCustom.confirm('External node request', 'Are you sure you want to connect to an ethereum node?', () => {
          modal.hide()
          modalDialogCustom.prompt('External node request', 'Web3 Provider Endpoint', 'http://localhost:8585', (target) => {
